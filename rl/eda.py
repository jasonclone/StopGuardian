...
def plot_state_visitation(df: pd.DataFrame, out_dir: str, label: str, pairs: List[Tuple[str, str]]):
    for (xcol, ycol) in pairs:
        try:
            ...
            # 2D histogram → heatmap with correct axis labels
            ...
            for t in range(max_steps):
                ...
                try:
                    ...
                    try:
                        ...
                        action = 0
                try:
                    from action import apply_action_safe
...
def plot_value_function_heatmaps_from_model(
    seed_dir: str,
    model_name: str,
    run_id: str,
    out_dir: str,
    ...
    """
    Generate value heatmaps for all sensible numeric column pairs and return a tuple:
      (success: bool, info: dict_or_error_message)

    - Returns (True, results_dict) on success where results_dict maps (xcol,ycol) -> (True, path) or (False, error).
    - Returns (False, "error message") on fatal failure.
    """
    try:
        results = {}
        ...
        # Gather numeric columns (excluding common non-state columns)
        numeric_cols = [c for c in combined.columns if pd.api.types.is_numeric_dtype(combined[c]) and c not in exclude]
        if len(numeric_cols) < 2:
            return False, "Not enough numeric state columns to build heatmaps"

        # Build ordered candidate pairs:
        candidate_pairs = []
        # 1) explicit STATE_HEATMAP_PAIRS if present
        for a, b in STATE_HEATMAP_PAIRS:
            if a in combined.columns and b in combined.columns and a not in exclude and b not in exclude:
                candidate_pairs.append((a, b))
        # 2) canonical NUM_FEATURES order (adjacent pairs)
        present_pref = [c for c in NUM_FEATURES if c in combined.columns and c not in exclude]
        for i in range(len(present_pref) - 1):
            pair = (present_pref[i], present_pref[i + 1])
            if pair not in candidate_pairs:
                candidate_pairs.append(pair)
        # 3) all unique combinations of numeric_cols (avoid duplicates)
        from itertools import combinations
        for a, b in combinations(numeric_cols, 2):
            if (a, b) not in candidate_pairs and (b, a) not in candidate_pairs:
                candidate_pairs.append((a, b))

        # Build a best-effort model once (try to use env.compute_state_size if available)
        model = None
        try:
            temp_state_size = 2
            try:
                from traffic_env import TrafficEnv  # optional
                env = TrafficEnv("C")
                try:
                    temp_state_size = env.compute_state_size()
                except Exception:
                    temp_state_size = 2
            except Exception:
                temp_state_size = 2
            ...
            if build_rainbow_model is not None:
                model, _ = build_rainbow_model(state_size=temp_state_size, lr_decay_steps=1)
            elif build_standard_model is not None:
                model, _ = build_standard_model(state_size=temp_state_size, lr_decay_steps=None)
        except Exception:
            model = None
        ...
        # Load checkpoint robustly onto CPU (attempt weights_only when available)
        try:
            try:
            ...
            return False, f"Failed reading checkpoint file: {e}"

        # Extract model state dict if nested
        if isinstance(ckpt_data, dict) and "model" in ckpt_data and isinstance(ckpt_data["model"], dict):
            ckpt_model = ckpt_data["model"]
        elif isinstance(ckpt_data, dict) and all(isinstance(v, (torch.Tensor, np.ndarray)) or hasattr(v, "shape") for v in ckpt_data.values()):
            ckpt_model = ckpt_data
        else:
            return False, "Checkpoint does not contain a model state_dict"

        # Move model to CPU and attempt loads (strict -> shape-filtered -> non-strict)
        try:
            model.to("cpu")
        except Exception:
            pass
        ...
        try:
            ...
            for k, v in ckpt_model.items():
                if isinstance(v, torch.Tensor):
                    ckpt_model_t[k] = v.to("cpu")
                else:
                    try:
                        ckpt_model_t[k] = torch.as_tensor(v).to("cpu")
                    except Exception:
                        ckpt_model_t[k] = v
            model.load_state_dict(ckpt_model_t)
            model.eval()
        except Exception as strict_err:
            # fallback: shape-matching partial load
            try:
                ...
                for k, v in ckpt_model.items():
                    try:
                        if isinstance(v, torch.Tensor):
                            v_shape = tuple(v.size())
                        elif isinstance(v, np.ndarray):
                            v_shape = tuple(v.shape)
                        else:
                            v_shape = tuple(np.array(v).shape)
                        if k in model_state and tuple(model_state[k].shape) == v_shape:
                            filtered[k] = v if isinstance(v, torch.Tensor) else torch.as_tensor(v)
                            filtered[k] = filtered[k].to("cpu")
                    except Exception:
                        continue
                ...
                if filtered:
                    ...
                    # last attempt: non-strict load with tensors only
                    try:
                        ckpt_tensors = {k: (torch.as_tensor(v).to("cpu") if not isinstance(v, torch.Tensor) else v.to("cpu"))
                                        for k, v in ckpt_model.items() if isinstance(v, (torch.Tensor, np.ndarray))}
                        model.load_state_dict(ckpt_tensors, strict=False)
                        model.eval()
                    except Exception as non_strict_err:
                        return False, f"Failed loading checkpoint: {strict_err}; fallback non-strict failed: {non_strict_err}"
            except Exception as partial_err:
                return False, f"Failed loading checkpoint: {strict_err}; fallback failed: {partial_err}"

        # Disable stochastic behavior
        try:
            model.eval()
            if hasattr(model, "disable_noise"):
                try:
                    model.disable_noise()
                except Exception:
                    pass
        ...
        # For each candidate pair, build grid, adapt to model input, run inference, save heatmap
        for (xcol, ycol) in candidate_pairs:
            try:
                if xcol not in combined.columns or ycol not in combined.columns:
                    results[(xcol, ycol)] = (False, "Columns not present")
                    continue
                ...
                if pd.isna(xmin) or pd.isna(xmax) or pd.isna(ymin) or pd.isna(ymax):
                    results[(xcol, ycol)] = (False, "Invalid numeric ranges")
                    continue
                ...
                # Prepare grid tensor on CPU
                grid_t = torch.from_numpy(grid).float().to("cpu")

                # infer expected input width from first 2D parameter
                sample_in = None
                for name, param in model.named_parameters():
                    if param.ndim == 2:
                        sample_in = param.shape[1]
                        break
                if sample_in is None:
                    sample_in = grid_t.shape[1]

                # adapt grid columns to model input width by padding/truncating
                ...
                fig, ax = plt.subplots(figsize=(8, 6))
                im = ax.imshow(Z, origin="lower", extent=(xmin, xmax, ymin, ymax), cmap="viridis", aspect="auto")
                ax.set_xlabel(xcol)
                ax.set_ylabel(ycol)
                ax.set_title(f"{model_name} — Value Heatmap ({os.path.basename(seed_dir)})\n{xcol} vs {ycol}")
                fig.colorbar(im, ax=ax, label="value")
                pair_fname = f"{os.path.basename(seed_dir)}_value_heatmap_{xcol}_{ycol}.png"
                out_path = os.path.join(out_dir, pair_fname)
                save_fig(fig, out_path)
                results[(xcol, ycol)] = (True, out_path)
            except Exception as e_pair:
                results[(xcol, ycol)] = (False, f"Inference/plot failed: {e_pair}")

        return True, results

    except Exception as e:
        try:
            log_exception(out_dir, "plot_value_function_heatmaps_from_model", e)
        except Exception:
            pass
        return False, f"plot_value_function_heatmaps_from_model error: {e}"


    
    

def plot_trajectory_if_positions(step_df: pd.DataFrame, out_dir: str, label: str, max_episodes: Optional[int] = None):
    """
    Save one PNG per episode containing a single subplot with both vehicle_queue and ped_queue time-series.
    - No trajectory/top subplot is created.
    - X axis: prefer 'step_in_episode', else index within episode.
    - If 'episode' exists, produce one file per episode (sorted). Limit with max_episodes.
    - If no episode column, produce a single combined queues PNG for the whole dataframe.
    - Returns (True, info_str) on success or (False, error_message) on failure.
    """
    try:
        if step_df is None or step_df.empty:
            return False, "Empty step_df"

        cols_lower = {c.strip().lower(): c for c in step_df.columns}

        # Detect queue column names (original-case)
        ped_queue_col = cols_lower.get("ped_queue")
        vehicle_queue_col = None
        for cand in ("vehicle_queue", "veh_queue", "vehicle_q", "veh_q"):
            if cand in cols_lower:
                vehicle_queue_col = cols_lower[cand]
                break
        if vehicle_queue_col is None and "vehicle_queue" in cols_lower:
            vehicle_queue_col = cols_lower["vehicle_queue"]

        # If neither queue exists, nothing to plot
        if ped_queue_col is None and vehicle_queue_col is None:
            return False, "No ped_queue or vehicle_queue columns found"

        ensure_dir(out_dir)

        # Helper to build x values for a group/dataframe
        def _get_x_vals_for_group(g: pd.DataFrame):
            if "step_in_episode" in cols_lower and cols_lower["step_in_episode"] in g.columns:
                return g[cols_lower["step_in_episode"]].astype(float).values, cols_lower["step_in_episode"]
            elif "global_step" in cols_lower and cols_lower["global_step"] in g.columns:
                return g[cols_lower["global_step"]].astype(float).values, cols_lower["global_step"]
            else:
                return np.arange(len(g)), "index_in_episode"

        # Per-episode files
        if "episode" in cols_lower:
            episode_col = cols_lower["episode"]
            grouped = list(step_df.groupby(episode_col))
            try:
                grouped.sort(key=lambda t: float(t[0]))
            except Exception:
                pass

            total_eps = len(grouped)
            limit = total_eps if max_episodes is None else min(total_eps, int(max_episodes))
            saved = 0

            for i, (ep_val, g) in enumerate(grouped):
                if i >= limit:
                    break
                ...
                try:
                    safe_ep = int(ep_val) if (pd.notna(ep_val) and float(ep_val).is_integer()) else str(ep_val)
                except Exception:
                    safe_ep = str(ep_val)

                x_vals, x_label = _get_x_vals_for_group(g)

                fig, ax = plt.subplots(figsize=(8, 4.5))
                plotted_any = False

                if vehicle_queue_col is not None and vehicle_queue_col in g.columns:
                    try:
                        y_veh = g[vehicle_queue_col].astype(float).values
                        ax.plot(x_vals, y_veh, lw=1.0, alpha=0.9, label=vehicle_queue_col, color="C0")
                        plotted_any = True
                    except Exception as e:
                        log_exception(out_dir, f"plot_vehicle_ep_{ep_val}", e)

                if ped_queue_col is not None and ped_queue_col in g.columns:
                    try:
                        y_ped = g[ped_queue_col].astype(float).values
                        ax.plot(x_vals, y_ped, lw=1.0, alpha=0.9, label=ped_queue_col, color="C1")
                        plotted_any = True
                    except Exception as e:
                        log_exception(out_dir, f"plot_ped_ep_{ep_val}", e)
                ...
                ax.set_ylabel("queue")
                ax.set_title(f"{label} — Queues ep{safe_ep}")
                ax.legend(ncol=2, fontsize="small")
                out_path = os.path.join(out_dir, f"{label}_ep{safe_ep}_queues.png")
                save_fig(fig, out_path)
                saved += 1

            if saved == 0:
                return False, "No per-episode queue plots saved (all groups empty or failed)"
            return True, f"Saved {saved} per-episode queue PNGs"
        else:
            # Single combined plot for whole dataframe
            x_vals, x_label = _get_x_vals_for_group(step_df)
            fig, ax = plt.subplots(figsize=(10, 4.5))
            plotted_any = False

            if vehicle_queue_col is not None and vehicle_queue_col in step_df.columns:
                try:
                    ax.plot(x_vals, step_df[vehicle_queue_col].astype(float).values, lw=1.0, alpha=0.9, label=vehicle_queue_col, color="C0")
                    plotted_any = True
                except Exception as e:
                    log_exception(out_dir, "plot_vehicle_whole", e)

            if ped_queue_col is not None and ped_queue_col in step_df.columns:
                try:
                    ax.plot(x_vals, step_df[ped_queue_col].astype(float).values, lw=1.0, alpha=0.9, label=ped_queue_col, color="C1")
                    plotted_any = True
                except Exception as e:
                    log_exception(out_dir, "plot_ped_whole", e)

            if not plotted_any:
                plt.close(fig)
                return False, "No queue data plotted for whole dataframe"

            ax.set_xlabel(x_label)
            ax.set_ylabel("queue")
            ax.set_title(f"{label} — Queues")
            ax.legend(ncol=2, fontsize="small")
            out_path = os.path.join(out_dir, f"{label}_queues.png")
            save_fig(fig, out_path)
            return True, "Saved combined queues PNG for whole dataframe"
    except Exception as e:
        try:
            log_exception(out_dir, "plot_trajectory_if_positions", e)
        except Exception:
            pass
            ...
            try:
                ...
                for idx, s in enumerate(seed_list):
                    ...
                    try:
                        ...
                        ok, info = plot_value_function_heatmaps_from_model(seed_dir, model_label, run_id, seed_out_dir, step_rows, grid_size=40)
                        if ok:
